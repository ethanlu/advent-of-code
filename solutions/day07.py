...
class Day07(object):
    def __init__(self, input_file):
        with open(input_file) as f:
            self._circuits = map(lambda l: self._parse_circuit(l.strip()), f.readlines())
    ...
    def _parse_circuit(self, x):
        unitary = re.match('(NOT\s)?([a-z0-9]+) \-> ([a-z]+)', x)
        binary = re.match('([a-z0-9]+) (AND|OR|RSHIFT|LSHIFT) ([a-z0-9]+) \-> ([a-z]+)', x)
    ...
    def _run_circuit(self):
        ...
        while circuits.qsize() > 0:
            ...
            try:
                ...
                circuits.put((operator, wire, sources))
