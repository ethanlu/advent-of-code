...
class Day18(object):
    def __init__(self, input_file):
        ...
        with open(input_file) as f:
            self._initial_setup = "".join([l.strip() for l in f.readlines()])
    ...
    def _num_on(self):
        on_states = 0
        for x in xrange(self._grid_dimension):
            for y in xrange(self._grid_dimension):
                if self._state[x][y]:
                    on_states += 1
...
if __name__ == '__main__':
    p = Day18('../../input/2015/day18.txt')

    print '-----part one-----'
    print p.part_one(100)

    print '-----part two-----'
    print p.part_two(100)